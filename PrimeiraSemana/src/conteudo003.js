...
var [firstItem, , guessThisItem, , fifthItem] = [
  "Carrots",
  "Carrot Bits",
  "Grass",
  "Berries",
...
function favThings({ cost, name, type }) {
  ...
  console.log(
    `Bunny loves her ${name}! She especially loves ${
      type[randomNum]
    } for only $${cost}!`
  );
}
...
function profilePage(
  { favColor } = { favColor: "vintage pink" },
  [name, age] = ["Bunny", 24]
) {
  console.log(
    `My name is ${name}. I am ${age} years old and my favorite color is ${favColor}!`
  );
}
profilePage();
